...
function prependTab($str, $count=1, $tabChar="&emsp;&emsp;&emsp;"){
   $tab = "";
...
function object2String($obj, $depth=1){
   ...
   //inject( print_r($arr, true), "pre" );
   
   foreach($arr as $key=>$val){
      if ( is_object($val) ) {
         $str .= prependTab("", $depth) . "[{$key}]: {<br>".  object2String($val, $depth+1);
      } else {
         $str .= prependTab("", $depth) . "[{$key}]: ". toString($val);
      }
      $str .= "<br>";
   }
   
   # '}'
   $str .= prependTab("", $depth-1) . '}<br>';
...
function prettify($arr, $format_assoc=False, $depth=0){
   ...
   ## Array, $format_assoc: false 
      ...
      if( empty($arr) ){
         ...
         _forEach(function($curr, $indx, $arr) use (&$str, $depth, $format_assoc, $BASE){
            $lastIndex = length($arr) - 1;
